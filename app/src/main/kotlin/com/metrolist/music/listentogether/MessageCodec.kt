...
class MessageCodec(
    ...
    private fun toProtoMessage(payload: Any): MessageLite {
        return when (payload) {
            ...
            is PlaybackActionPayload -> {
                ...
                payload.trackId?.let<String, Any> { builder.setTrackId(it) }
                payload.trackInfo?.let<TrackInfo, Any> { builder.setTrackInfo(trackInfoToProto(it)) }
                payload.queueTitle?.let<String, Any> { builder.setQueueTitle(it) }
        ...
        return when (msgType) {
            ...
            MessageTypes.SYNC_PLAYBACK -> {
                ...
                PlaybackActionPayload(
                    ...
                    queue = pb.queueList.map { track: Listentogether.TrackInfo -> protoToTrackInfo(track) },
                    queueTitle = pb.queueTitle.takeIf { it.isNotEmpty() },
                    volume = pb.volume.takeIf { pb.action == PlaybackActions.SET_VOLUME },
                    serverTime = pb.serverTime.takeIf { it > 0L },
                    revision = pb.revision,
                    capturedAtServerTime = pb.capturedAtServerTime.takeIf { it > 0L },
                )
            }
    ...
    private fun protoToRoomState(proto: Listentogether.RoomState): RoomState {
                return RoomState(
                    ...
                    hostId = proto.hostId,
                    users = proto.usersList.map { user: Listentogether.UserInfo -> protoToUserInfo(user) },
                    ...
                    volume = proto.volume,
                    queue = proto.queueList.map { track: Listentogether.TrackInfo -> protoToTrackInfo(track) },
